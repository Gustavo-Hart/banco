...
window.addEventListener("loade", /*saldoInicial(),*/ atualizaSaldoDeposito(),atualizaSaldoSaque());
...
//função: atualiza o saldo após um saque
function atualizaSaldoSaque() {
    const saldoAfterSaque = localStorage.getItem('afterSaque');

    if (saldoAfterSaque) {
        localStorage.setItem('saldoAtual', saldoAfterSaque)


        const myConta1 = new contaBancaria();
        myConta1._saldo = saldoAfterSaque;
        conta1.splice(2,1, myConta1._saldo);
